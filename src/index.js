import { buildJSONData, Generator } from './dungGen/Generator.js'
...
document.getElementById('download').addEventListener('click', () => {
  if (generator) {
    const object = buildJSONData()
    console.log(object)
    const a = document.createElement('a')
    a.setAttribute('href', object)
    a.setAttribute('download', 'dungeon.json')
    a.click()
  }
})
...
document.getElementById('configSubmit').addEventListener('click', async () => {
  ...
  generator = await Generator(
    ...
    (generator) => {
      ...
      let i = 0
      Object.values(generator.corridors).forEach((c) =>
