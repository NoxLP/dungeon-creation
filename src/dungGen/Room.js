...
export function Room(topLeft, bottomRight) {
  ...
  this.center = simpleGetProxy([
    Math.floor((bottomRightCoords[0] - topLeftCoords[0]) / 2) +
      topLeftCoords[0],
    Math.floor((bottomRightCoords[1] - topLeftCoords[1]) / 2) +
      topLeftCoords[1],
  ])
  ...
  this.isInside = (coords) =>
    coords[0] >= topLeftCoords[0] &&
    coords[0] <= bottomRightCoords[0] &&
    coords[1] >= topLeftCoords[1] &&
    coords[1] <= bottomRightCoords[1]
  ...
  this.isInMargin = (coords) =>
    (coords[1] <= this.yMax &&
      coords[1] >= this.yMin &&
      (this.xMax + 1 == coords[0] || this.xMin - 1 == coords[0])) ||
    (coords[0] <= this.xMax &&
      coords[0] >= this.xMin &&
      (this.yMax + 1 == coords[1] || this.yMin - 1 == coords[1])) ||
    (this.xMin - 1 == coords[0] && this.yMin - 1 == coords[1]) ||
    (this.xMax + 1 == coords[0] && this.yMin - 1 == coords[1]) ||
    (this.xMax + 1 == coords[0] && this.yMax + 1 == coords[1]) ||
    (this.xMin - 1 == coords[0] && this.yMax + 1 == coords[1])
